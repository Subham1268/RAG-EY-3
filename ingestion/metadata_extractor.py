...
async def extract_metadata_from_document(content: str, file_path: Path) -> dict:
    """Call GPT-4o to infer metadata from the document's first pages."""
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",   # cheaper for metadata extraction
            temperature=0,
            response_format={"type": "json_object"},
            messages=[{"role": "user", "content": METADATA_PROMPT.format(content=content[:3000])}]
        )
        meta = eval(response.choices[0].message.content)   # or json.loads
        # Sanity and fallbacks
        ...
        return {"engagement_id": "UNKNOWN", "client": "Unknown", "country": "GCC", "practice": "General", "year": 2024}